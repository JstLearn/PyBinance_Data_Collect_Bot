...
def get_klines(coinname, intervals_sql, intervals):
    ...
    for a in range(len(intervals_sql)):
        ...
        try:  # DB'de last timestamp varsa son eklenenden günümüze kadar alıcaz
            cursor.execute(
                "select top (1) Open_time from [Binance].[dbo].[Klines] where symbol=(?) and interval=(?) order by Open_time desc",
                coinname, intervals_sql[a])
            ...
            for i in range(len(bars_since_last_log)):
                ...
                db.commit()
            print(
                f"{coinname} çiftinin {intervals_sql[a]} barlarının {pd.to_datetime(last_timestamp, unit='ms')} Tarihinden itibaren {len(bars_since_last_log)} Adet verisini SQL Klines Tablosuna {round(time.time() - intervaltime, 3)} Saniyede ekledim {datetime.datetime.now().strftime('%H:%M:%S')}")
            ...
            for bar in range(len(bars_since_listed)):
                ...
                db.commit()
            print(
                f"{coinname} çiftinin {intervals_sql[a]} barlarının başlangıç tarihinden itibaren {len(bars_since_listed)} Adet verisini SQL Klines Tablosuna {round(time.time() - intervaltime, 3)} Saniyede ekledim {datetime.datetime.now().strftime('%H:%M:%S')}")
    if round(time.perf_counter(), 3) > 3:
        print(
            f"{coinname} çiftinin {intervals_sql} Barları tarandı {round(time.perf_counter(), 3)} Saniye sürdü {datetime.datetime.now().strftime('%H:%M:%S')}")
    return 1
...
if __name__ == '__main__':
    # coin_update()
    cursor.execute(
        "SELECT TOP (1000) [symbol] FROM [Binance].[dbo].[PairSpecs] where status = 'TRADING' and quoteAsset = 'USDT' order by symbol")
    coinnames = tuple(map(lambda x: x[0], cursor.fetchall()))
    for coin in coinnames:  # while yapacaksan çok dikkatli ol bilgisayarı çökertme
        if coinnames.index(coin) == 0:  # başa döndüğünü buradan anlıyoruz
            intervals = (
            '1MONTH', '1w', '3d', '1d', '12h', '8h', '6h', '4h', '2h', '1h', '30m', '15m', '5m', '3m', '1m')
            intervals_duration = (
            2678400, 604800, 259200, 86400, 43200, 28800, 21600, 14400, 7200, 3600, 1800, 900, 300, 180, 60)
            stamp_now = int(time.time())
            intervals_sql = []
            for i in range(len(intervals)):
                if stamp_now > last_rendered_timestamps[i] + intervals_duration[
                    i]:  # last_rendered_timestamps[i] her işleme girdiğinde yeni değeri almalı
        ...
        print('fonksiyonun dışındayım')


        def again(a):
            print('fonkiyona girdim aynı anda taradığım coin sayısı = ', a)
            time.sleep(10)
            if a <= 2:
                a += 1
                print('simultaneus arttırdım = ', a)
                response = multiprocessing.Process(target=get_klines, args=(coin, intervals_sql, intervals)).start()
                if response == 1:
                    a -= response
                    print('fonkiyondan çıktım aynı anda taradığım coin sayısı = ', a)
            else:
                again(a)
        ...
        # Sırayla coin çekme işlemi
        # response = get_klines(coin, intervals_sql, intervals)
        ...
        #         multiprocessing.Process(target=get_klines, args=(coin, intervals_sql, intervals)).start()
        #     else:
        #         again()
        # again()
