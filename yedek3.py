...
def get_klines(coinname):
    ...
    if round(time.perf_counter(), 3) > 3:
        print(f"{intervals_sql} Tarandı {round(time.perf_counter(), 3)} Saniye sürdü {datetime.datetime.now().strftime('%H:%M:%S')}")
...
if __name__ == '__main__':
    for pair in coinnames[2:3]:
        multiprocessing.Process(target=get_klines, args=[pair]).start()
"""